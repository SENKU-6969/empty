...
function renderPrediction(pred, tests) {
  ...
  return `<div class="insight-card"><span class="insight-emoji">🔮</span><div class="insight-title">Predictive Score</div>
...
function renderTopicClusters(clusters) {
  if (!clusters.length) return `<div class="insight-card"><span class="insight-emoji">🎯</span><div class="insight-title">Smart Topic Association</div><div class="insight-desc" style="margin-top:8px;">No weakness patterns detected yet. Fill in Deep Dive weakness logs to enable this feature.</div></div>`;
...
function renderGhostSummary(tests) {
  ...
  return `<div class="insight-card"><span class="insight-emoji">👻</span><div class="insight-title">Ghost Mistake History</div>
    <div class="insight-desc">Subjects where you consistently spend too much time relative to marks scored.</div>
    ${!sorted.length?`<div style="margin-top:12px;color:var(--neon-green);font-size:13px;">✅ No time management issues detected!</div>`:sorted.map(([s,c])=>`<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid var(--border);"><span style="font-weight:600;color:var(--text-primary);">${s}</span><span style="color:var(--neon-red);font-family:'JetBrains Mono';font-size:13px;">⚡ ${c} test${c>1?'s':''}</span></div>`).join('')}
  </div>`;
}
...
function renderFormulaDeckInsight(formulas) {
  ...
  return `<div class="insight-card"><span class="insight-emoji">🧮</span><div class="insight-title">Formula Deck Status</div>
...
function renderRecentActivity(sorted) {
  ...
  return `<div class="insight-card full"><span class="insight-emoji">📅</span><div class="insight-title">Recent Activity</div>
    <div style="margin-top:16px;">${recent.map((t,i) => {
      ...
      return `<div style="display:flex;align-items:center;gap:16px;padding:12px 0;${i<recent.length-1?'border-bottom:1px solid var(--border)':''};cursor:pointer;" onclick="location.href='deep-dive.html?id=${t.id}'">
        ...
        <div style="flex:1;"><div style="font-weight:600;font-size:14px;">${t.name}</div><div style="font-size:12px;color:var(--text-secondary);">${formatDate(t.date)} · ${t.marks}/${t.maxMarks}${t.rank?' · Rank #'+t.rank:''}</div>${t.weaknesses?`<div style="font-size:11px;color:var(--text-muted);margin-top:2px;">⚠️ ${t.weaknesses.slice(0,60)}${t.weaknesses.length>60?'…':''}</div>`:''}</div>
